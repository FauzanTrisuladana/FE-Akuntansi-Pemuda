...
export function PengurusEditDialog({
  ...
  useEffect(() => {
    if (open && pengurus) {
      ...
      setSelesaiMenjabat(
        pengurus.selesaiMenjabat == null
          ? ""
          : String(pengurus.selesaiMenjabat),
      );
      setStatus(pengurus.status);
    }
  }, [open, pengurus]);

  const selectedJabatan = jabatanOptions.find(
    (item) => String(item.id) === jabatanId,
  );
  const currentJabatanOption = pengurus
    ? (jabatanOptions.find(
        (item) => String(item.id) === String(pengurus.jabatanId),
      ) ?? {
  ...
  const editJabatanOptions = currentJabatanOption
    ? [
        ...
        ...jabatanOptions.filter(
          (item) => String(item.id) !== String(currentJabatanOption.id),
        ),
      ]
    : jabatanOptions;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await onEdit({
        ...
        selesaiMenjabat:
          selesaiMenjabat.trim() === "" ? null : Number(selesaiMenjabat),
        status,
      });
        ...
        <DialogForm onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">
              Edit Pengurus Koperasi
            </DialogTitle>
            <DialogDescription>
              Silakan ubah data pengurus koperasi
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogBody className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              {jabatanError ? (
                <p className="text-sm text-destructive">{jabatanError}</p>
              ) : null}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                />
                {mulaiError ? (
                  <p className="text-sm text-destructive">{mulaiError}</p>
                ) : null}
              </div>
              <div className="grid gap-2">
                <Label
                  htmlFor="edit-pengurus-selesai"
                  className="text-slate-600 font-medium"
                >
                  Selesai Menjabat
                </Label>
                ...
                />
                {selesaiError ? (
                  <p className="text-sm text-destructive">{selesaiError}</p>
                ) : null}
              </div>
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Select
                value={status}
                onValueChange={(value) =>
                  setStatus(value as "aktif" | "selesai")
                }
              >
                <SelectTrigger
                  id="edit-pengurus-status"
                  className="h-auto min-h-12 w-full px-4 py-3 text-left"
                >
