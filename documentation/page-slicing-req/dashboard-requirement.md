...
Dokumen ini memuat spesifikasi teknis, fungsi, dan tipe visualisasi grafik yang dibutuhkan pada Dashboard Keuangan. Spesifikasi ini disusun berdasarkan struktur database (*Entity-Relationship Diagram*) dan rencana kebutuhan pelaporan organisasi.

---

## 1. Komponen Informasi Ringkasan (Statistik Utama)
Pada bagian paling atas dashboard, sistem harus menampilkan tiga kartu informasi (*summary cards*) untuk memberikan gambaran cepat mengenai kondisi keuangan berjalan:
...
* **Judul Grafik:** `Tren Pertumbuhan Saldo per Jenis Kas`
* **Fungsi Visualisasi:** Menampilkan beberapa jalur garis tren sekaligus dalam satu bidang grafik untuk memantau performa masing-masing kas. Jumlah garis yang muncul bersifat dinamis mengikuti isi data master pada tabel `akun` (berdasarkan kolom enum `kas`).
    * *Contoh:* Menampilkan Garis Jalur 1 untuk pergerakan saldo **Kas 17-an**, dan Garis Jalur 2 untuk pergerakan saldo **Kas Pemuda**.
...
1.  **Format Kebersihan Data Antarmuka:** Pengembang hanya berfokus pada implementasi tipe grafik dan kedinamisan judul di atas. Abaikan teks keterangan statis, label persentase perbandingan bulan lalu, atau teks *footer* bawaan komponen chart generik pada tahap *slicing layout*.
2.  **Rekomendasi Struktur Baris:**
    ...
    * **Baris 3 (Bawah):** Grafik `Tren Pertumbuhan Saldo per Jenis Kas` (Multi-Line Chart) bersanding di sebelah grafik `Log Rekonsiliasi Kas (Aplikasi vs Riil)` (Multi-Bar Chart).
